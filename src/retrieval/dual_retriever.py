...
class DualRetriever:
    ...
    async def retrieve(self, sub_queries: list[SubQuery]) -> list[RetrievalResult]:
        if self._use_chromadb:
            results = await self._retrieve_chromadb(sub_queries)
        else:
            results = await self._retrieve_bruteforce(sub_queries)
        await self._enrich_low_confidence_results(results, sub_queries)
        return results

    # ========== DAG expansion ==========

    async def _expand_dag_context(self, hit_query_ids: list[str]) -> list[list[VersionChainNode]]:
        ...
        for start_qid in hit_query_ids:
            ...
            ancestors = await self._bfs_ancestors(start_qid)
            if len(ancestors) <= 1:
                continue
    ...
    async def _bfs_ancestors(self, start_query_id: str) -> list[VersionChainNode]:
        visited: set[str] = set()
        ...
        while queue and current_depth < self.version_chain_depth:
            ...
            nodes = await self.memory_store.get_fake_queries_by_ids(batch_ids)
    ...
    async def _retrieve_chromadb(self, sub_queries: list[SubQuery]) -> list[RetrievalResult]:
        store: ChromaDBStore = self.memory_store

        # --- Phase 1: Batch Q2Q search (single ChromaDB call for all sub-queries) ---
        valid_sqs = [sq for sq in sub_queries if sq.embedding is not None]
        if not valid_sqs:
            return []

        all_embeddings = [sq.embedding for sq in valid_sqs]
        batch_results = await store.batch_search_fake_queries(
            all_embeddings, top_k=self.top_k_per_sub
        )

        candidate_map: dict[str, dict] = {}
        matched_fq_details: dict[str, list[dict]] = {}

        for sq, fq_hits in zip(valid_sqs, batch_results):
            for hit in fq_hits:
                mid = hit["memory_id"]
        ...
        # --- Phase 1.5: DAG Expansion (concurrent per candidate) ---
        dag_contexts: dict[str, list[list[VersionChainNode]]] = {}

        async def _expand_for_mid(mid: str, details: list[dict]):
            hit_qids = [h["query_id"] for h in details if h.get("parent_ids")]
            if hit_qids:
                dag_contexts[mid] = await self._expand_dag_context(hit_qids)
            else:
                dag_contexts[mid] = []

        await asyncio.gather(*[
            _expand_for_mid(mid, details)
        ...
        # --- Phase 3: Q2C rerank (batch per candidate, concurrent across candidates) ---
        if len(sorted_candidates) <= self.top_n:
            results = await self._build_results(store, sorted_candidates, dag_contexts)
            logger.info(
        ...
        valid_embeddings = [sq.embedding for sq in sub_queries if sq.embedding is not None]

        async def _q2c_score_for(mid: str, info: dict):
            best_q2c = await store.batch_search_content_for_memory(
                valid_embeddings, memory_id=mid, top_k=self.top_k_q2c,
            )
            info["score_q2c"] = best_q2c

        await asyncio.gather(*[
            _q2c_score_for(mid, info) for mid, info in sorted_candidates
        ])
        ...
        results = await self._build_results(store, top_candidates, dag_contexts)
        logger.info(
    ...
    async def _retrieve_bruteforce(self, sub_queries: list[SubQuery]) -> list[RetrievalResult]:
        memories = await self.memory_store.load_all()
        if not memories:
            logger.warning("Memory store is empty, no results to retrieve")
            return []

        # CPU-heavy scoring → offload to thread
        def _score_all():
            candidate_map: dict[str, dict] = {}
            matched_fq_details: dict[str, list[dict]] = {}
            ...
            return candidate_map, matched_fq_details

        candidate_map, matched_fq_details = await asyncio.to_thread(_score_all)
        ...
        # --- Phase 3: Q2C rerank ---
        if len(sorted_candidates) <= self.top_n:
            results = await self._build_results(None, sorted_candidates, dag_contexts)
            logger.info(
        ...
        await asyncio.to_thread(_q2c_score_all)

        sorted_candidates.sort(key=lambda x: x[1]["final_score"], reverse=True)
        top_candidates = sorted_candidates[:self.top_n]

        results = await self._build_results(None, top_candidates, dag_contexts)
        logger.info(
    ...
    async def _build_results(
        ...
        # Batch-fetch memories that need loading from store
        mids_to_fetch = [
            mid for mid, info in candidates
            if "memory" not in info and store is not None
        ]
        fetched = {}
        if mids_to_fetch and store:
            memories = await asyncio.gather(*[store.get_by_id(mid) for mid in mids_to_fetch])
            for mid, mem in zip(mids_to_fetch, memories):
                if mem:
                    fetched[mid] = mem
    ...
    async def _enrich_low_confidence_results(
        ...
        async def _enrich_single(r: RetrievalResult):
            best_paragraphs: dict[str, float] = {}
            for sq in sub_queries:
                if sq.embedding is None:
                    continue
                hits = await self.memory_store.search_paragraphs_for_memory(
        ...
        await asyncio.gather(*[_enrich_single(r) for r in results])
